...
- [ ] **Messaging V1 — Phase 4a (Send Safety, mock-only)** implemented locally behind `ENABLE_MESSAGING` flag and ready for staging QA (2026-04-26). Quota (`msg_quota` 50/500 placeholders, manual top-up via helper), suppression (`msg_suppression`, manual add/remove), two-step confirmation (`msg_send_confirmations`, single-use token, 5-min expiry), atomic quota increment with decrement-on-failure, history view. All sends route through `MockProvider`. ZERO real SMS, ZERO 019/external/admin route. Packet: `MISSIONS/messaging_v1_packet_04.md`. Next: Phase 4b (019 test connector, separate approval).
