...
- [ ] **Messaging V1 — Phase 5 (One controlled real SMS via 019 production endpoint)** implemented locally and ready for staging QA (2026-04-27). New `_ONENINE_PROD_URL` constant; real-mode branch in `OneNineProvider.send()` gated by 5 layers: `ONENINE_USE_REAL=1` + positive staging allowlist (`APP_BASE_URL` contains `zman-emet-platform.onrender.com`) + `ONENINE_PILOT_RECIPIENT` set + destination equality + hardened numeric source validation. Any failure → no network call. NO new routes. NO UI change. Default provider stays `mock`. The actual pilot send is a separate manual operator step requiring CEO approval; this packet only ships the code. Packet: `MISSIONS/messaging_v1_packet_05.md`.
