...
- [ ] **Messaging V1 — Phase 4b (019 test endpoint adapter)** live on staging behind `ENABLE_MESSAGING=1` (2026-04-27). `OneNineProvider` real implementation against `https://019sms.co.il/api/test` ONLY. Default provider stays `mock`; 4a UI unchanged. Packet: `MISSIONS/messaging_v1_packet_04b.md`.
- [ ] **Messaging V1 — Phase 5 (One controlled real SMS via 019 production endpoint)** live on staging behind `ENABLE_MESSAGING=1` (2026-04-27). Real-mode branch dormant (no `ONENINE_USE_REAL=1` set anywhere; no SMS sent). Direction switched to Twilio pilot via Phase 5T. Packet: `MISSIONS/messaging_v1_packet_05.md`.
- [ ] **Messaging V1 — Phase 5T (Twilio pilot adapter)** implemented locally and ready for staging QA (2026-04-27). `TwilioProvider` real implementation with 5-layer real-mode gate: `TWILIO_USE_REAL=1` + positive staging allowlist (`APP_BASE_URL` contains `zman-emet-platform.onrender.com`) + `TWILIO_PILOT_RECIPIENT` set + destination equality + hardened E.164 `TWILIO_FROM` validation. NO real-mode branch active (no `TWILIO_USE_REAL=1` set anywhere). Twilio reachable only via shell-invoked `dispatch_send_message(... provider_name="twilio")`. NO new routes; NO UI change; default provider stays `mock`; 4a UI continues to call mock; 019 adapter untouched. Pilot send requires two separate CEO approvals (activate flag + execute send). Packet: `MISSIONS/messaging_v1_packet_05t.md`.
